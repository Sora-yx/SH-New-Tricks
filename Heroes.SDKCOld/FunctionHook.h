...
public:
	...
	{
		if (hookaddr)
			throw std::exception("Cannot apply hook to already hooked function!");
		memcpy(origbytes, origaddr, 5);
		hookaddr = hook;
		WriteJump(origaddr, hook);
	}
	...
	TRet Original(TArgs... args)
	{
		if (hookaddr)
		{
			...
			memcpy(hookbytes, origaddr, 5);
			memcpy(origaddr, origbytes, 5);
			TRet retval = origaddr(args...);  // Call original function
			memcpy(origaddr, hookbytes, 5);
			return retval;
		}
		else
		{
			return origaddr(args...);  // Direct call if no hook is applied
		}
	}
